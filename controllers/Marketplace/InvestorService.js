...
* @Last Modified time: 2018-03-09 16:08:34
*/
...
module.exports.postAdviceForInvestorGroup = function(args, res, next) { 
	...
	.then(([advice, investor]) => {
		if(advice && investor) {
			...
			if(operation == "accept") {
				return Promise.all([
					InvestorModel.acceptInvitationFromAdvice({_id: investorId}, adviceId),
					AdviceModel.addInvestorToGroup({_id: adviceId}, investorId)
				]);	
			} else if(operation == "reject") {
				return InvestorModel.rejectInvitationFromAdvice({_id: investorId}, adviceId);	
